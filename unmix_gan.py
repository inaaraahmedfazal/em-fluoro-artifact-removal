...
class UnmixGAN():
    ...
    def __init__(self, args, dataloaders):
        ...
        self._gan_loss = loss.GANLoss(gan_mode='lsgan').to(device)
    ...
    def _load_checkpoint(self):
        ...
        if os.path.exists(os.path.join(self.checkpoint_dir, 'last_ckpt.pt')):
            ...
            self.epoch_to_start = checkpoint['epoch_id']
            self.epoch_to_start = 30
            if checkpoint['epoch_id']>2:
                self.best_val_acc = checkpoint['best_val_acc']+10
                self.best_train_acc = self.best_val_acc
            else:
    ...
    def _collect_running_batch_states(self):
        ...
        if self.is_training is True:
                self.training_acc = np.mean(self.running_acc)
        elif self.is_training is False:
                self.validation_acc = np.mean(self.running_acc)
    ...
    def _update_checkpoints(self):
        ...
        if self.metric == 'labrmse_gt1':
            ...
            #if self.validation_acc < self.best_val_acc:
                #self.best_val_acc = self.validation_acc
                #self.best_epoch_id = self.epoch_id
                #self.best_train_acc = 0.25*self.training_acc+0.75*self.validation_acc
                #self._save_checkpoint(ckpt_name='best_ckpt.pt')
                #print('*' * 10 + 'Best model updated1!')
                #print()
            if self.validation_acc*0.95+self.training_acc*0.05 < self.best_train_acc:
                # a lower score is better
                self.best_train_acc = self.validation_acc*0.95+self.training_acc*0.05
                self.best_epoch_id = self.epoch_id
                self.best_val_acc = self.validation_acc
                self._save_checkpoint(ckpt_name='best_ckpt.pt')
...
#                self.best_val_acc = self.validation_acc
#                self.best_epoch_id = self.epoch_id
#                self.best_train_acc = 0.05*self.training_acc+0.95*self.validation_acc
#                self._save_checkpoint(ckpt_name='best_ckpt.pt')
#                print('*' * 10 + 'Best model updated2!')
#                print()
            if self.validation_acc*0.95+self.training_acc*0.05 > self.best_train_acc:
        ...
        # update the best model (based on eval acc)
        print('Lastest model updated. Best_train_acc=%.4f, Historical_best_val_acc=%.4f (at epoch %d), Training_acc=%.4f, Val_acc=%.4f'
              % (self.best_train_acc, self.best_val_acc, self.best_epoch_id,self.training_acc,self.validation_acc))
        print()

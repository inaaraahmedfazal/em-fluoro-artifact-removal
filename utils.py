...
class Syn3Dataset(Dataset):
    ...
    def __init__(self, root_dir, subset, img_size=512, is_train=True):
        ...
        if subset == 'defocused':
            ...
            self.dirs = glob.glob(os.path.join(self.root_dir,'C', '*.png'))
    ...
    def __getitem__(self, idx):
        ...
        this_gt_dir2 = this_dir.replace('/C', '/A')
        gt2 = cv2.imread(this_gt_dir2, cv2.IMREAD_COLOR)
        gt2 = cv2.cvtColor(gt2, cv2.COLOR_BGR2RGB)
...
def get_loaders(args):
    ...
    if args.dataset == 'dogs':
        ...
        training_set = Rain800Dataset(
            ...
            img_size=512, suff='.png', gt_left=True, is_train=True)
        val_set = Rain800Dataset(
            root_dir=r'./datasets/Rain800/val',
            img_size=512, suff='.png', gt_left=True, is_train=False)

    elif args.dataset == 'did-mdn':
        training_set = Rain800Dataset(
            root_dir=r'./datasets/DID-MDN/train',
            img_size=512, suff='.png', gt_left=False, is_train=True)
        val_set = Rain800Dataset(
            root_dir=r'./datasets/DID-MDN/val',
            img_size=512, suff='.png', gt_left=False, is_train=False)
        ...
        training_set = Syn3Dataset(
            #root_dir=r'./datasets/Syn3/train', img_size=512, is_train=True)
            root_dir=r'./datasets/Syn3/train', subset='all', img_size=512, is_train=True)
        val_set = Syn3Dataset(
            #root_dir=r'./datasets/Syn3/val', img_size=512, is_train=False)
...
def cpt_ssim(batch, batch_gt):
    ...
    for i in range(m):
        ...
        #ssim_index, ssim_map = cal_ssim(img[:,:,1], gt[:,:,1])
        #ssim = np.mean(ssim_map)
        ssim = compare_ssim(img[:,:,1], gt[:,:,1])
        SSIM = SSIM + ssim
